...
URL_HIST_PRICE_DAY = 'https://min-api.cryptocompare.com/data/histoday?fsym={}&tsym={}&limit={}&e={}'
URL_HIST_PRICE_HOUR = 'https://min-api.cryptocompare.com/data/histohour?fsym={}&tsym={}&limit={}&toTs={}&e={}'
...
def get_historical_price_day(coin, curr=CURR, limit=LIMIT, exchange='CCCAGG', quiet=True):
...
def get_historical_price_hour(coin, curr=CURR, limit=LIMIT, timestamp=time.time(), exchange='CCCAGG', quiet=True):
    if isinstance(timestamp, datetime.datetime):
        timestamp = time.mktime(timestamp.timetuple())
    url = URL_HIST_PRICE_HOUR.format(coin, format_parameter(curr), limit, int(timestamp), format_parameter(exchange))
    return query_cryptocompare(url, quiet=quiet)
